...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            critic_info = {"Baseline Loss": float(np.mean(critic_losses))}
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounts = self.gamma ** np.arange(len(rewards), dtype=np.float32)
        ret = float(np.sum(discounts * rewards))
        return np.full_like(rewards, ret, dtype=np.float32)
