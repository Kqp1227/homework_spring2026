...
class MLPPolicy(nn.Module):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.discrete:
            # Return a Python int for Gym discrete action spaces.
            return int(action_t.cpu().numpy().squeeze())

        # Continuous control: return a NumPy array with shape (ac_dim,)
        action_np = action_t.cpu().numpy()
        if single:
            action_np = action_np[0]  # (ac_dim,)
        return action_np
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(
        ...
        if self.discrete:
            act_long = actions.long().squeeze(-1) if actions.ndim > 1 else actions.long()
            logp = dist.log_prob(act_long)
        else:
