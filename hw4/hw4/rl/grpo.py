    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                adv_per_token = adv.unsqueeze(1)  # [B_mb, 1] -> broadcast to [B_mb, L-1]

                unclipped = ratio * adv_per_token
                clipped = torch.clamp(ratio, 1.0 - cfg.clip_eps, 1.0 + cfg.clip_eps) * adv_per_token
                per_token_obj = torch.min(unclipped, clipped) * mask

                seq_obj_i = masked_mean_per_row(per_token_obj, mask)
